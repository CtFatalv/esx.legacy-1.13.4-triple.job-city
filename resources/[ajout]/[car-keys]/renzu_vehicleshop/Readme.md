...
RegisterNUICallback("choosebrands", function(data, cb)
    local vehtable = {}
    local cars = 0
    local selectedCategory = data.brand 
    local cats = {}

    for k, v2 in pairs(Vehicles) do
        for k2, v in pairs(v2) do
            -- On vérifie si la catégorie correspond et si le shop est correct
            if selectedCategory == v.category and IsModelInCdimage(GetHashKey(v.model)) and (id == v.shop) then
                cars = cars + 1
                ...
                local veh = {
                    brand = v.category, -- On utilise la catégorie comme étiquette
                    category = v.category,
                    image = v.image,
                    name = v.name,
                    brake = v.brake,
...
-- N'oublie pas de rajouter la fonction de fermeture si elle manque à la fin de ton fichier :
RegisterNUICallback("close", function(data, cb)
    ...
    DoScreenFadeOut(500)
    Wait(500)
    SetEntityCoords(ped, shopcoords)
    DoScreenFadeIn(1000)
    cb('ok')
end)
...
-- ==========================================
-- 1. DÉFINITION DE LA GESTION DES DONNÉES
-- ==========================================
...
-- ==========================================
-- 2. DÉFINITION DE L'OUVERTURE DU SHOP
-- ==========================================
function OpenShop(id, v)
    ...
    -- Variable de secours pour éviter le nil à la fin
    local shopcoords = vector3(v.shop_x, v.shop_y, v.shop_z)
    ...
    if not Config.Quickpick then
        ...
        while not fetchdone do Wait(100) end
        ...
        for catName, vehicleList in pairs(Vehicles) do
            for _, vehData in pairs(vehicleList) do
                if (id == vehData.shop) then
                    local hasAccess = (vehData.job == playerJob and playerGrade >= vehData.grade) or (vehData.job == false)
                    
                    if hasAccess then
                        brands[catName] = catName
                        if vehtable[vehData.name] == nil then
                            vehtable[vehData.name] = {}
                        end
                        table.insert(vehtable[vehData.name], vehData)
                    end
                end
            end
        end

        SendNUIMessage({ brands = brands, type = "brands" })
        SendNUIMessage({ garage_id = id, data = vehtable, type = "display" })
        
        SetNuiFocus(true, true)

        -- Gestion de la Caméra
        RequestCollisionAtCoord(2800.5966796875,-3799.7370605469,139.41514587402)
        for k, shopData in pairs(VehicleShop) do
            local dist = #(vector3(shopData.shop_x, shopData.shop_y, shopData.shop_z) - GetEntityCoords(ped))
            
            if Config.UseArenaSpawn then
                local vec = vector3(2800.5966796875,-3799.7370605469,139.41514587402)
                dist = #(vec - GetEntityCoords(ped))
                if dist <= 40.0 and id == shopData.name then
                    shopcoords = vector3(shopData.shop_x, shopData.shop_y, shopData.shop_z)
                ...
                if dist <= 40.0 and id == shopData.name then
                    shopcoords = vector3(shopData.shop_x, shopData.shop_y, shopData.shop_z)
                    if shopData.type == "car" then
                        cam = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", shopData.shop_x-5.0, shopData.shop_y-3.0, shopData.shop_z-28.5, 360.00, 0.00, 0.00, 60.00, false, 0)
                        PointCamAtCoord(cam, shopData.shop_x, shopData.shop_y, shopData.shop_z-30.0)
                    elseif shopData.type == 'boat' then
                        cam = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", shopData.shop_x-5.0, shopData.shop_y-3.0, shopData.shop_z-29.0, 360.00, 0.00, 0.00, 60.00, false, 0)
                        PointCamAtCoord(cam, shopData.shop_x, shopData.shop_y, shopData.shop_z-40.0)
                    else
                        cam = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", shopData.shop_x-13.0, shopData.shop_y-10.0, shopData.shop_z-30.0, 360.00, 0.00, 0.00, 60.00, false, 0)
                        PointCamAtCoord(cam, shopData.shop_x, shopData.shop_y, shopData.shop_z-40.0)
                    end
                    ...
                    SetCamFov(cam, 45.0)
                    RenderScriptCams(true, true, 1, true, true)
                    SetFocusPosAndVel(shopData.shop_x, shopData.shop_y, shopData.shop_z-30.0, 0.0, 0.0, 0.0)
                    DisplayHud(false)
                    DisplayRadar(false)
                    break
                end
        ...
        -- Thread pour les lumières et rotation
        Citizen.CreateThread(function()
        ...
        -- Ce bloc s'exécute si Config.Quickpick est TRUE
...
-- ==========================================
-- 3. CALLBACKS NUI (CLIC CATÉGORIE)
-- ==========================================
RegisterNUICallback("choosecategory", function(data, cb)
    local vehtable = {}
    local selectedCategory = data.category or data.id
    local playerData = ESX.GetPlayerData()
    local playerJob = playerData.job.name
    local playerGrade = playerData.job.grade

    if Vehicles[selectedCategory] then
        for _, v in pairs(Vehicles[selectedCategory]) do
            local hasAccess = (v.job == playerJob and playerGrade >= v.grade) or (v.job == false)
            if hasAccess then
                if vehtable[v.name] == nil then vehtable[v.name] = {} end
                table.insert(vehtable[v.name], v)
            end
        end
    end
...
-- ON ENVOIE LES DONNÉES AU MENU (NUI)
    SendNUIMessage({
        brands = brands, 
        type = "brands"
    })
    ...
    -- Début de la logique de prévisualisation
   if not Config.Quickpick then
        -- On lance la logique de caméra
        local foundShop = false
        
        for k,v in pairs(VehicleShop) do
            -- ... (ta boucle for pour trouver le shop)
            foundShop = true -- Si on trouve le shop, on passe à true
        end

        if foundShop then 
            -- ICI : Tout ce qui se passe si le shop est trouvé (caméras, lumières, etc.)
            
            Citizen.CreateThread(function()
                -- Ta boucle de lumière
            end)

        else -- CE 'ELSE' EST MAINTENANT RATTACHÉ AU 'IF foundShop'
            FreezeEntityPosition(PlayerPedId(), true)
            if shopcoords then 
                SetEntityCoords(PlayerPedId(), shopcoords) 
            end
            ...
            FreezeEntityPosition(PlayerPedId(), false)
            ShowNotification("No Vehicle is Available 2")
        end -- FIN DU IF foundShop
        
    end -- FIN DU IF not Config.Quickpick
...
RegisterNUICallback("SpawnVehicle", function(data, cb)
    -- On vérifie si le modèle arrive via 'modelcar' ou simplement 'model'
    local model = data.modelcar or data.model
    
    if model then
        if not Config.Quickpick then
            SpawnVehicleLocal(model)
        end
    end
